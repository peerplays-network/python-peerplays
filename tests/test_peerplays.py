...
class Testcases(unittest.TestCase):
    ...
    def test_create_account(self):
        ...
        role = "active"
        self.assertIn(
            format(key2.pubkey, "TEST"),
            [x[0] for x in op[role]["key_auths"]])
        self.assertIn(
            format(key4.pubkey, "TEST"),
            [x[0] for x in op[role]["key_auths"]])
        self.assertIn(
            "1.2.0",
    ...
    def test_update_memo_key(self):
        tx = peerplays.update_memo_key("TEST55VCzsb47NZwWe5F3qyQKedX9iHBHMVVFSc96PDvV7wuj7W86n")
        self.assertEqual(
            getOperationNameForId(tx["operations"][0][0]),
            "account_update"
        )
